...
# wandb.init(entity="suimang", project="ky_predictor_pad", name="padv2_lr2.0e-4_5")
...
def preprocess(mp4_paths, star_frame, kp_detector, pad, frames=64, device='cuda'):
    ...
    return kpjacobians, imgs, paddings
def calculate_loss( kpjacobians, gen_kp, paddings, loss_function, interation, istrain):
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, dic in enumerate(train_data):
            ...
            kpjacobians, imgs, paddings = preprocess(mp4_paths=mp4_paths, star_frame=star_frame, kp_detector=kp_detector, pad=pad)
            t = {}
            ...
            gen_kp = audio2kp(t)
            train_interation += 1
            loss = calculate_loss(kpjacobians, gen_kp, paddings, loss_function, train_interation, istrain=True)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
